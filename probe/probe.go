...
func Probe() {
	logger.Info("Start to probe alive machines")
	common.AliveHosts = CheckLive(common.RunningInfo.Hosts) // 不应该在这调用
}

func CheckLive(hostslist []string) []string {
	if common.RunningInfo.NoProbe {
	...
	go func() {
		for ip := range chanHosts {
			if _, ok := ExistHosts[ip]; !ok && utils.IsContain(hostslist, ip) {
				...
				if common.RunningInfo.Silent == false {
					output := fmt.Sprintf("Target %-10s is alive", ip)
	...
	if err == nil {
		...
		if err == nil {
			...
			utils.LogError(err)
			//使用ping探测
			logger.Info("The current user permissions unable to send icmp packets")
	...
	if len(hostslist) > 1000 {
		arrTop, arrLen := ArrayCountValueTop(AliveHosts, common.RunningInfo.LiveTop, true)
	...
	if len(hostslist) > 256 {
		arrTop, arrLen := ArrayCountValueTop(AliveHosts, common.RunningInfo.LiveTop, false)
		for i := 0; i < len(arrTop); i++ {
	...
	logger.Info(
		logger.LightGreen("There are total of ") +
			logger.White(strconv.Itoa(len(hostslist))) +
			logger.LightGreen(" hosts, and ") +
			logger.White(strconv.Itoa(len(AliveHosts))) +
