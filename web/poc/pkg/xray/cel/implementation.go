...
import (
	...
	"hscan/web/poc/internal/common/errors"
	"hscan/web/poc/pkg/xray/requests"
	"hscan/web/poc/pkg/xray/structs"
	"hscan/web/poc/utils"
...
func xrayNewReverse() (reverse *structs.Reverse) {
	...
	reverse.ReverseType = common.RunningInfo.ReversePlatformType
...
func reverseCheck(r *structs.Reverse, timeout int64) bool {
	...
	case structs.ReverseType_Ceye:
		...
		urlStr := fmt.Sprintf("http://api.ceye.io/v1/records?token=%s&type=dns&filter=%s", common.RunningInfo.CeyeApi, sub)
	...
	case structs.ReverseType_DnslogCN:
		time.Sleep(time.Second * time.Duration(timeout))
		sub := strings.Split(r.Domain, ".")[0]
		resp, _, err := requests.DoRequest(common.RunningInfo.DnslogCNGetRecordRequest, false)
		if err != nil {
			wrappedErr := errors.Wrap(err, "Reverse check error")
			utils.ErrorP(wrappedErr)
			return false
