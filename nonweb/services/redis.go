...
import (
	...
	"fmt"
	"hscan/common"
	nonweb_utils "hscan/nonweb/utils"
	"hscan/utils"
	"hscan/utils/logger"
	"net"
	"os"
...
func RedisScan(info nonweb_utils.HostInfo) (tmperr error) {
	starttime := time.Now().Unix()
	flag, err := RedisUnauth(info.Host, info.Port)
	...
	for _, pass := range common.Passwords {
		...
		flag, err := RedisConn(info.Host, info.Port, pass)
		if flag == true && err == nil {
			return err
		} else {
			errlog := fmt.Sprintf("redis %v:%v %v %v", info.Host, info.Port, pass, err)
			...
			if time.Now().Unix()-starttime > (int64(len(common.Passwords)) * int64(common.RunningInfo.BruteTimeout)) {
				return err
			}
		}
	}
...
func RedisConn(Host, Port, pass string) (flag bool, err error) {
	flag = false
	realhost := fmt.Sprintf("%s:%v", Host, Port)
	conn, err := utils.WrapperTcpWithTimeout("tcp", realhost, time.Duration(common.RunningInfo.BruteTimeout)*time.Second)
	defer func() {
		if conn != nil {
			conn.Close()
		}
	...
	err = conn.SetReadDeadline(time.Now().Add(time.Duration(common.RunningInfo.BruteTimeout) * time.Second))
	if err != nil {
		return flag, err
	}
...
func RedisUnauth(Host, Port string) (flag bool, err error) {
	flag = false
	realhost := fmt.Sprintf("%s:%v", Host, Port)
	conn, err := utils.WrapperTcpWithTimeout("tcp", realhost, time.Duration(common.RunningInfo.BruteTimeout)*time.Second)
	defer func() {
