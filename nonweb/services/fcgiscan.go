...
func FcgiScan(info nonweb_utils.HostInfo) {
	...
	addr := fmt.Sprintf("%v:%v", info.Host, info.Port)
	...
	if err != nil {
		errlog := fmt.Sprintf("[-] fcgi %v:%v %v", info.Host, info.Port, err)
	...
	if strings.Contains(string(stdout), cutLine) { //命令成功回显
		...
		if len(stderr) > 0 {
			result = fmt.Sprintf("[+] FCGI:%v:%v \n%vstderr:%v\nplesa try other path,as -path /www/wwwroot/index.php", info.Host, info.Port, output, string(stderr))
		} else {
			result = fmt.Sprintf("[+] FCGI:%v:%v \n%v", info.Host, info.Port, output)
		}
		...
		if len(stderr) > 0 {
			result = fmt.Sprintf("[+] FCGI:%v:%v \n%vstderr:%v\nplesa try other path,as -path /www/wwwroot/index.php", info.Host, info.Port, string(stdout), string(stderr))
		} else {
			result = fmt.Sprintf("[+] FCGI:%v:%v \n%v", info.Host, info.Port, string(stdout))
		}
		logger.Success(result)
	}
}
...
func New(addr string, timeout int64) (fcgi *FCGIClient, err error) {
	conn, err := utils.WrapperTcpWithTimeout("tcp", addr, time.Duration(timeout)*time.Second)
	fcgi = &FCGIClient{
		rwc:       conn,
		keepAlive: false,
	}
